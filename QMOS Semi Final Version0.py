...
# حالا برنامه‌های کاربردی را تعریف می‌کنیم
class QuantumPhone:
...
class NeuralFileSystem:
    ...
    def refresh_files(self):
        ...
        try:
            ...
            if parent_dir != self.current_path:  # Not root
                self.tree.insert('', 'end', values=('..', '', 'Parent Directory', ''), tags=('folder',))
            ...
            for item in os.listdir(self.current_path):
                ...
                try:
                    if os.path.isdir(item_path):
                        ...
                        self.tree.insert('', 'end', values=(item, size, file_type, modified), tags=('folder',))
                        ...
                        modified = datetime.fromtimestamp(os.path.getmtime(item_path)).strftime('%Y-%m-%d %H:%M')
                        self.tree.insert('', 'end', values=(item, size, file_type, modified), tags=('file',))
            ...
            self.tree.tag_configure('folder', foreground='#00ffff')
            self.tree.tag_configure('file', foreground='white')
            
            self.path_label.config(text=self.current_path)
            self.status_label.config(text=f"Loaded {len(self.file_list)} items")
            
        except PermissionError:
            messagebox.showerror("Permission Denied", "Cannot access this directory")
            self.go_back()
    ...
    def open_file(self):
        ...
        try:
            if os.path.isdir(file_path):
                ...
                # Log file access
                self.log_file_access(file_path)
    ...
    def delete_file(self):
        ...
        try:
            if os.path.isdir(file_path):
                if messagebox.askyesno("Confirm Delete", f"Delete folder '{self.selected_file}' and all its contents?"):
                    shutil.rmtree(file_path)
                    self.status_label.config(text=f"Deleted folder: {self.selected_file}")
            else:
                if messagebox.askyesno("Confirm Delete", f"Delete file '{self.selected_file}'?"):
                    os.remove(file_path)
                    self.status_label.config(text=f"Deleted file: {self.selected_file}")
    ...
    def show_properties(self):
        ...
        try:
            ...
            is_dir = "Yes" if os.path.isdir(file_path) else "No"
...
Permissions: {oct(stats.st_mode)[-3:]}
            ...
            messagebox.showerror("Error", f"Cannot get file properties: {str(e)}")
    ...
    def log_file_access(self, file_path):
        ...
        try:
            ...
            self.os.q_cursor.execute('''
                INSERT INTO file_history (filename, path, file_type, size, last_accessed)
                VALUES (?, ?, ?, ?, ?)
            ''', (filename, file_path, file_type, size, timestamp))
...
class QuantumMusic:
    ...
    def show(self):
        ...
        album_frame = tk.Frame(self.window, bg='#0a0a0a')
        album_frame.pack(pady=20)
        
        self.album_canvas = tk.Canvas(album_frame, width=200, height=200, 
                                    bg='#1a1a1a', highlightthickness=0)
        self.album_canvas.pack()
        self.draw_album_art()
        
        # Track info
        track_frame = tk.Frame(self.window, bg='#0a0a0a')
        track_frame.pack(pady=10)
        
        self.track_label = tk.Label(track_frame, text=self.tracks[self.current_track], 
                                  font=('Arial', 12, 'bold'), bg='#0a0a0a', fg='white')
        self.track_label.pack()
        
        self.artist_label = tk.Label(track_frame, text="Quantum AI Generated", 
        ...
        tk.Label(time_frame, text="0:00", font=('Arial', 8), 
                bg='#0a0a0a', fg='#cccccc').pack(side='left')
        tk.Label(time_frame, text="3:45", font=('Arial', 8), 
        ...
        prev_btn = tk.Button(controls_frame, text="⏮", font=('Arial', 14),
                           bg='#252525', fg='white', width=3,
        ...
        next_btn = tk.Button(controls_frame, text="⏭", font=('Arial', 14),
                           bg='#252525', fg='white', width=3,
        ...
        tk.Label(playlist_frame, text="Quantum Playlist:", font=('Arial', 10, 'bold'),
    ...
    def draw_album_art(self):
        """Draw animated album art"""
        self.album_canvas.delete("album")
        
        # Create quantum-inspired album art
        ...
        for i in range(3):
            radius = 30 + i * 20
            angle = time_factor * (i + 1) * 0.5
            x = center_x + radius * math.cos(angle)
            y = center_y + radius * math.sin(angle)
            
            color = ['#00ffff', '#ff00ff', '#ffff00'][i]
            self.album_canvas.create_oval(x-15, y-15, x+15, y+15,
                                        fill=color, outline='', tags="album")
        
        # Pulsing center
        pulse_size = 20 + math.sin(time_factor * 3) * 5
        self.album_canvas.create_oval(center_x-pulse_size, center_y-pulse_size,
                                    center_x+pulse_size, center_y+pulse_size,
                                    fill='#00ff88', outline='', tags="album")
        
        if self.playing:
            self.window.after(50, self.draw_album_art)
    ...
    def simulate_progress(self):
        if self.playing:
            ...
            if current < 100:
                self.progress['value'] = current + 0.5
                self.window.after(100, self.simulate_progress)
            else:
                self.next_track()
    ...
    def update_track_info(self):
        self.track_label.config(text=self.tracks[self.current_track])
        self.draw_album_art()
...
class NeuralHealth:
    ...
    def show(self):
        ...
        self.hr_label = tk.Label(hr_frame, text="72 BPM", font=('Arial', 14),
                               bg='#1a1a1a', fg='white')
        self.hr_label.pack(anchor='w')
        
        # Blood Pressure
        bp_frame = tk.Frame(metrics_frame, bg='#1a1a1a')
        bp_frame.pack(fill='x', pady=5)
        tk.Label(bp_frame, text="🩸 Blood Pressure", font=('Arial', 12, 'bold'),
                bg='#1a1a1a', fg='#ff8844').pack(anchor='w')
        self.bp_label = tk.Label(bp_frame, text="120/80 mmHg", font=('Arial', 14),
                               bg='#1a1a1a', fg='white')
        self.bp_label.pack(anchor='w')
        
        # Oxygen Level
        oxy_frame = tk.Frame(metrics_frame, bg='#1a1a1a')
        oxy_frame.pack(fill='x', pady=5)
        tk.Label(oxy_frame, text="🫁 Oxygen", font=('Arial', 12, 'bold'),
                bg='#1a1a1a', fg='#4488ff').pack(anchor='w')
        self.oxy_label = tk.Label(oxy_frame, text="98%", font=('Arial', 14),
                                bg='#1a1a1a', fg='white')
        self.oxy_label.pack(anchor='w')
        
        # Sleep Quality
        sleep_frame = tk.Frame(metrics_frame, bg='#1a1a1a')
        sleep_frame.pack(fill='x', pady=5)
        tk.Label(sleep_frame, text="😴 Sleep Quality", font=('Arial', 12, 'bold'),
                bg='#1a1a1a', fg='#8844ff').pack(anchor='w')
        self.sleep_label = tk.Label(sleep_frame, text="7h 30m (85%)", font=('Arial', 14),
                                  bg='#1a1a1a', fg='white')
        self.sleep_label.pack(anchor='w')
        
        # Activity
        activity_frame = tk.Frame(metrics_frame, bg='#1a1a1a')
        activity_frame.pack(fill='x', pady=5)
        tk.Label(activity_frame, text="🏃 Activity", font=('Arial', 12, 'bold'),
                bg='#1a1a1a', fg='#44ff88').pack(anchor='w')
        self.activity_label = tk.Label(activity_frame, text="8,234 steps", font=('Arial', 14),
                                     bg='#1a1a1a', fg='white')
        self.activity_label.pack(anchor='w')
        
        # Neural Activity
        neural_frame = tk.Frame(metrics_frame, bg='#1a1a1a')
        neural_frame.pack(fill='x', pady=5)
        tk.Label(neural_frame, text="🧠 Neural Activity", font=('Arial', 12, 'bold'),
                bg='#1a1a1a', fg='#ff44ff').pack(anchor='w')
        self.neural_label = tk.Label(neural_frame, text="Optimal", font=('Arial', 14),
                                   bg='#1a1a1a', fg='white')
        self.neural_label.pack(anchor='w')
    ...
    def update_health_metrics(self):
        ...
        # Simulate realistic health data with small variations
        heart_rate = 70 + random.randint(-3, 3)
        systolic = 118 + random.randint(-5, 5)
        diastolic = 78 + random.randint(-3, 3)
        oxygen = 97 + random.randint(0, 1)
        sleep_hours = 7 + random.randint(0, 1)
        sleep_minutes = 15 + random.randint(0, 45)
        steps = 8234 + random.randint(-100, 100)
        
        self.hr_label.config(text=f"{heart_rate} BPM")
        self.bp_label.config(text=f"{systolic}/{diastolic} mmHg")
        self.oxy_label.config(text=f"{oxygen}%")
        self.sleep_label.config(text=f"{sleep_hours}h {sleep_minutes}m (8{random.randint(3,7)}%)")
        self.activity_label.config(text=f"{steps:,} steps")
        
        neural_states = ["Optimal", "Stable", "Active", "Calm"]
        self.neural_label.config(text=random.choice(neural_states))
        
        if self.window.winfo_exists():
            self.window.after(3000, self.update_health_metrics)
...
class QuantumFinance:
    ...
    def show(self):
        ...
        tk.Label(portfolio_frame, text="Portfolio Value", font=('Arial', 12),
                bg='#0a0a0a', fg='#cccccc').pack(anchor='w')
        self.portfolio_label = tk.Label(portfolio_frame, text="$154,832.47", 
                                      font=('Arial', 24, 'bold'), bg='#0a0a0a', fg='#00ff88')
        self.portfolio_label.pack(anchor='w')
        
        change_frame = tk.Frame(portfolio_frame, bg='#0a0a0a')
        change_frame.pack(anchor='w')
        self.change_label = tk.Label(change_frame, text="+$2,847.32 (+1.87%)", 
        ...
        indices = [
            ("QFI (Quantum Index)", "15,482.67", "+1.24%"),
            ("NASQ (Neural Tech)", "8,742.31", "+2.15%"),
            ("AIX (AI Sector)", "3,458.92", "+0.87%"),
            ("QTUM (Quantum Tech)", "1,247.85", "+3.42%")
        ]
        
        for name, value, change in indices:
            index_frame = tk.Frame(indices_frame, bg='#1a1a1a')
            index_frame.pack(fill='x', pady=2)
            
            tk.Label(index_frame, text=name, font=('Arial', 10),
                   bg='#1a1a1a', fg='white').pack(side='left', padx=10)
            tk.Label(index_frame, text=value, font=('Arial', 10, 'bold'),
                   bg='#1a1a1a', fg='#00ffff').pack(side='left', padx=10)
            
            change_color = '#00ff88' if '+' in change else '#ff4444'
            tk.Label(index_frame, text=change, font=('Arial', 10),
                   bg='#1a1a1a', fg=change_color).pack(side='right', padx=10)
        
        # Investment recommendations
        rec_frame = tk.Frame(self.window, bg='#0a0a0a')
        rec_frame.pack(fill='both', expand=True, padx=20, pady=10)
        
        tk.Label(rec_frame, text="Quantum Recommendations:", font=('Arial', 12, 'bold'),
                bg='#0a0a0a', fg='#ff44ff').pack(anchor='w')
        
        recommendations = [
            "⚡ Quantum Computing Inc. - STRONG BUY",
            "🧠 Neural Networks Ltd. - BUY", 
            "🔮 Photon Technologies - HOLD",
            "⚛️  Quantum AI Corp - STRONG BUY",
            "🌌 Temporal Systems - BUY"
        ]
        
        for rec in recommendations:
            tk.Label(rec_frame, text=rec, font=('Arial', 10),
                   bg='#0a0a0a', fg='#cccccc', anchor='w').pack(anchor='w', pady=2)
        
        # Trading buttons
        trade_frame = tk.Frame(self.window, bg='#0a0a0a')
        trade_frame.pack(fill='x', padx=20, pady=10)
        
        buy_btn = tk.Button(trade_frame, text="QUANTUM BUY", font=('Arial', 12),
                          bg='#00aa00', fg='white', width=12)
        buy_btn.pack(side='left', padx=5)
        
        sell_btn = tk.Button(trade_frame, text="NEURAL SELL", font=('Arial', 12),
                           bg='#aa0000', fg='white', width=12)
        sell_btn.pack(side='left', padx=5)
        
        analyze_btn = tk.Button(trade_frame, text="AI ANALYZE", font=('Arial', 12),
                              bg='#0088aa', fg='white', width=12)
    ...
    def update_finance_data(self):
        """Update finance data with realistic fluctuations"""
        # Simulate market movements
        base_value = 154832.47
        fluctuation = random.uniform(-0.02, 0.03)  # -2% to +3%
        new_value = base_value * (1 + fluctuation)
        change = new_value - base_value
        change_percent = (change / base_value) * 100
        
        self.portfolio_label.config(text=f"${new_value:,.2f}")
        
        if change >= 0:
            self.change_label.config(text=f"+${change:,.2f} (+{change_percent:.2f}%)", 
                                   fg='#00ff88')
        else:
            self.change_label.config(text=f"-${abs(change):,.2f} ({change_percent:.2f}%)", 
                                   fg='#ff4444')
        
        if self.window.winfo_exists():
            self.window.after(5000, self.update_finance_data)
...
class SystemControl:
    ...
    def show(self):
        ...
        status_items = [
            ("Quantum Core", "98.7%", "🟢 Optimal"),
            ("Neural Networks", "95.2%", "🟢 Stable"), 
            ("AI Processors", "99.1%", "🟢 Optimal"),
            ("Temporal Sync", "99.9%", "🟢 Perfect"),
            ("Quantum Encryption", "100%", "🟢 Active"),
            ("Holographic Display", "0%", "🔴 Offline")
        ]
        
        for name, value, status in status_items:
            item_frame = tk.Frame(status_frame, bg='#1a1a1a')
            item_frame.pack(fill='x', pady=2)
            
            tk.Label(item_frame, text=name, font=('Arial', 11),
                   bg='#1a1a1a', fg='white').pack(side='left', padx=10)
            tk.Label(item_frame, text=value, font=('Arial', 11, 'bold'),
                   bg='#1a1a1a', fg='#00ffff').pack(side='left', padx=10)
            tk.Label(item_frame, text=status, font=('Arial', 11),
        ...
        # Row 1
        row1 = tk.Frame(controls_frame, bg='#0a0a0a')
        row1.pack(fill='x', pady=5)
        
        optimize_btn = tk.Button(row1, text="OPTIMIZE SYSTEM", font=('Arial', 11),
                               bg='#00aa00', fg='white', height=2,
                               command=self.optimize_system)
        optimize_btn.pack(side='left', expand=True, padx=2)
        
        diagnostics_btn = tk.Button(row1, text="RUN DIAGNOSTICS", font=('Arial', 11),
                                  bg='#0088aa', fg='white', height=2,
                                  command=self.run_diagnostics)
        diagnostics_btn.pack(side='left', expand=True, padx=2)
        
        # Row 2
        row2 = tk.Frame(controls_frame, bg='#0a0a0a')
        row2.pack(fill='x', pady=5)
        
        backup_btn = tk.Button(row2, text="QUANTUM BACKUP", font=('Arial', 11),
                             bg='#8844ff', fg='white', height=2,
                             command=self.quantum_backup)
        backup_btn.pack(side='left', expand=True, padx=2)
        
        security_btn = tk.Button(row2, text="SECURITY SCAN", font=('Arial', 11),
                               bg='#ff8800', fg='white', height=2,
                               command=self.security_scan)
        security_btn.pack(side='left', expand=True, padx=2)
        
        # Row 3
        row3 = tk.Frame(controls_frame, bg='#0a0a0a')
        row3.pack(fill='x', pady=5)
        
        restart_btn = tk.Button(row3, text="RESTART SYSTEM", font=('Arial', 11),
                              bg='#ff4444', fg='white', height=2,
                              command=self.restart_system)
        restart_btn.pack(side='left', expand=True, padx=2)
        
        shutdown_btn = tk.Button(row3, text="SHUTDOWN", font=('Arial', 11),
                               bg='#aa0000', fg='white', height=2,
                               command=lambda: ShutdownScreen(self.os).show())
        shutdown_btn.pack(side='left', expand=True, padx=2)
        
        # System logs
        logs_frame = tk.Frame(self.window, bg='#0a0a0a')
        logs_frame.pack(fill='both', expand=True, padx=20, pady=10)
        
        tk.Label(logs_frame, text="System Logs:", font=('Arial', 11, 'bold'),
                bg='#0a0a0a', fg='#ffff00').pack(anchor='w')
        
        log_text = scrolledtext.ScrolledText(logs_frame, height=8,
                                           bg='#1a1a1a', fg='#00ff88',
                                           font=('Arial', 9))
        log_text.pack(fill='both', expand=True)
        log_text.insert(tk.END, self.get_system_logs())
        log_text.config(state=tk.DISABLED)
    
    def optimize_system(self):
        messagebox.showinfo("System Optimization", 
                          "Quantum optimization initiated!\n\n"
                          "Neural networks: TUNED\n"
                          "Quantum coherence: MAXIMIZED\n"
                          "System performance: OPTIMAL")
    
    def run_diagnostics(self):
        messagebox.showinfo("System Diagnostics", 
                          "Quantum diagnostics complete!\n\n"
                          "All systems: NOMINAL\n"
                          "Performance: 99.8%\n"
                          "Security: 100%\n"
                          "Stability: EXCELLENT")
    
    def quantum_backup(self):
        messagebox.showinfo("Quantum Backup", 
                          "Neural quantum backup initiated!\n\n"
    ...
    def security_scan(self):
        messagebox.showinfo("Security Scan", 
                          "Quantum security scan complete!\n\n"
                          "Threat level: ZERO\n"
                          "Encryption: ACTIVE\n"
                          "Firewall: OPTIMAL\n"
                          "Neural defense: ENGAGED")
    
    def restart_system(self):
        if messagebox.askyesno("Restart System", 
                             "Restart Quantum OS?\n\n"
                             "All applications will close.\n"
                             "Unsaved data may be lost."):
            messagebox.showinfo("System Restart", 
                              "Quantum restart sequence initiated!\n\n"
                              "Neural networks: PRESERVED\n"
                              "Quantum state: MAINTAINED\n"
                              "System will restart in 3 seconds...")
    
    def get_system_logs(self):
        return """[SYSTEM] Quantum OS v3.0 initialized
[NEURAL] Neural networks online - 99.8% efficiency
[QUANTUM] Quantum coherence established
[AI] AI processors engaged - Optimal performance
[SECURITY] Quantum encryption active
[TEMPORAL] Time synchronization perfect
[SYSTEM] All services running normally
...
class ShutdownScreen:
    ...
    def animate_shutdown(self):
        ...
        # Draw pulsing quantum core
        pulse = math.sin(time.time() * 3) * 10 + 40
        self.canvas.create_oval(center_x-pulse, center_y-pulse,
                              center_x+pulse, center_y+pulse,
                              fill='#ff4444', outline='', tags="shutdown")
        
        # Draw collapsing orbitals
        for i in range(3):
            radius = max(0, 60 + i * 30 - self.shutdown_progress * 2)
            if radius > 0:
                self.canvas.create_oval(center_x-radius, center_y-radius,
                                      center_x+radius, center_y+radius,
                                      outline='#ff8844', width=2, tags="shutdown")
        
        # Draw shutdown text
        self.canvas.create_text(center_x, center_y - 100,
                              text="QUANTUM OS", font=('Arial', 16, 'bold'),
                              fill='#ff4444', tags="shutdown")
        
        self.canvas.create_text(center_x, center_y - 70,
                              text="Shutting down...", font=('Arial', 12),
                              fill='#ff8844', tags="shutdown")
        
        # Draw progress
        progress_width = 300
        progress_x = (self.os.screen_width - progress_width) // 2
        progress_y = center_y + 50
        
        self.canvas.create_rectangle(progress_x, progress_y,
                                   progress_x + progress_width, progress_y + 12,
                                   outline='#333333', fill='#1a1a1a', width=2, tags="shutdown")
        
        if self.shutdown_progress > 0:
            self.canvas.create_rectangle(progress_x, progress_y,
                                       progress_x + (progress_width * self.shutdown_progress) // 100, progress_y + 12,
                                       fill='#ff4444', outline='', tags="shutdown")
        
        self.canvas.create_text(self.os.screen_width // 2, progress_y + 30,
                              text=f"{self.shutdown_progress}%", font=('Arial', 12, 'bold'),
                              fill='#ff4444', tags="shutdown")
        
        # Update progress
        if self.shutdown_progress < 100:
            self.shutdown_progress += 2
            self.window.after(50, self.animate_shutdown)
        else:
            self.window.after(1000, self.finalize_shutdown)
    ...
    def finalize_shutdown(self):
        ...
        self.canvas.create_text(center_x, center_y,
                              text="System Safe to Shut Down\n\nQUANTUM OS v3.0",
                              font=('Arial', 14, 'bold'), fill='#00ff88',
                              justify='center', tags="shutdown")
        
        self.window.after(3000, self.close_all)
