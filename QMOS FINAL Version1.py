...
class ShutdownScreen:
    ...
    def animate_shutdown(self):
        ...
        if self.animation_stage < 50:
            # مرحله 1: جمع شدن ذرات کوانتومی
            for i in range(20):
                angle = (self.animation_stage * 0.1) + i * 0.314
                radius = 100 - self.animation_stage * 2
                x = center_x + radius * math.cos(angle)
                y = center_y + radius * math.sin(angle)
                
                size = 3 + math.sin(self.animation_stage * 0.2 + i) * 2
                color = ['#ff4444', '#ff8844', '#ffff44'][i % 3]
                
                self.canvas.create_oval(x-size, y-size, x+size, y+size,
                                      fill=color, outline='', tags="shutdown")
        
        elif self.animation_stage < 100:
            # مرحله 2: انفجار و پراکندگی
            explosion_radius = (self.animation_stage - 50) * 5
            for i in range(30):
                ...
                size = random.uniform(1, 4)
                color = random.choice(['#ff4444', '#ff0000', '#aa0000'])
            ...
            self.canvas.create_text(center_x, center_y - 50,
                                  text="SYSTEM SHUTDOWN", font=('Arial', 16, 'bold'),
                                  fill='#ff4444', tags="shutdown")
            
            self.canvas.create_text(center_x, center_y,
                                  text="Quantum OS v4.0\nPowered Off",
                                  font=('Arial', 12), fill='#ff8844',
                                  justify='center', tags="shutdown")
            
            self.canvas.create_text(center_x, center_y + 50,
                                  text="Created by Saleh Amoo",
                                  font=('Arial', 10), fill='#cccccc',
                                  tags="shutdown")
            
            self.window.after(2000, self.final_shutdown)
            return
